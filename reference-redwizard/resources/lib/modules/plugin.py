...
from .menus import main_menu, build_menu, submenu_maintenance, backup_restore, restore_gui_skin, kodi_settings, kodi_specific, kodi_builtins, addon_specific, addonbrowser
...
def mdblist():
    ...
    if xbmcvfs.exists(chk_fentastic) and xbmcvfs.exists(chkset_fentastic) and skin_chk == 'skin.fentastic':
        ...
        with open(chkset_fentastic) as f:
                    if your_mdb_api in f.read():
                        xbmcgui.Dialog().notification(addon_name, 'MDBlist Key Already Applied to Skin!!', addon_icon, 3000)
                    else:
                        xbmc.executebuiltin("Skin.SetString(mdblist_api_key, %s)" % your_mdb_api)
                        xbmcgui.Dialog().notification(addon_name, 'MDBlist Key Applied to Fentastic!!', addon_icon, 3000)
    ...
    if xbmcvfs.exists(chk_nimbus) and xbmcvfs.exists(chkset_nimbus) and skin_chk == 'skin.nimbus':
        ...
        with open(chkset_nimbus) as f:
                    if your_mdb_api in f.read():
                        ...
                        xbmc.executebuiltin("Skin.SetString(mdblist_api_key, %s)" % your_mdb_api)
                        xbmcgui.Dialog().notification(addon_name, 'MDBlist Key Applied to Nimbus!!', addon_icon, 3000)
    ...
    if xbmcvfs.exists(chk_altus) and xbmcvfs.exists(chkset_altus) and skin_chk == 'skin.altus':
        ...
        with open(chkset_altus) as f:
                    if your_mdb_api in f.read():
                        ...
                        xbmc.executebuiltin("Skin.SetString(mdblist_api_key, %s)" % your_mdb_api)
                        xbmcgui.Dialog().notification(addon_name, 'MDBlist Key Applied to Altus Skin!!', addon_icon, 3000)
    ...
    if mode is None:
        ...
        if not xbmcaddon.Addon("plugin.program.theredwizard").getSetting("mdb.api.key"):
            ...
            if ask:
                ...
                data = dialog.input('Enter your MDBlist API Key', type=xbmcgui.INPUT_ALPHANUM)
                if data:
